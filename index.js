...
// ---------------------------------------------------------------------------
// BIRTHDAYS
// Collected via DM: a month select menu, then a day select menu (split into
// two if the month has more than 25 days, since Discord caps select menus
// at 25 options). Stored as { userId: { month, day } }, persisted in the
// same hidden storage channel as the VIP list. A daily check announces
// matches in BIRTHDAY_ANNOUNCE_CHANNEL_ID.
// ---------------------------------------------------------------------------
const BIRTHDAY_ANNOUNCE_CHANNEL_ID = process.env.BIRTHDAY_ANNOUNCE_CHANNEL_ID;
let USER_BIRTHDAYS = {}; // userId -> { month: 1-12, day: 1-31 }
...
// Tracks who's been picked as VIP and when, most-recent-first. Capped at 30
// entries (well beyond the 15 actually displayed) to keep storage small.
let VIP_HISTORY = []; // [{ name, date }, ...] most recent first
let vipHistoryMessage = null;
const VIP_HISTORY_CAP = 30;
...
async function saveVipHistoryToDiscord() {
  if (!VIP_STORAGE_CHANNEL_ID) return;
  const content = 'VIPHISTORY:' + VIP_HISTORY.map(h => `${h.name}|${h.date}`).join(',');
  try {
    if (vipHistoryMessage) {
      await vipHistoryMessage.edit(content);
    } else {
      const channel = await client.channels.fetch(VIP_STORAGE_CHANNEL_ID);
      if (channel) vipHistoryMessage = await channel.send(content);
    }
  } catch (err) {
    console.error('Failed to save VIP history to Discord:', err.message);
  }
}

// ---------------------------------------------------------------------------
// BIRTHDAY PERSISTENCE - chunked across multiple messages (BIRTHDAYS0:,
// BIRTHDAYS1:, ...) since Discord's long user IDs mean a single message
// can't safely hold birthdays for a full ~90-member alliance.
// ---------------------------------------------------------------------------
async function saveBirthdaysToDiscord() {
  if (!VIP_STORAGE_CHANNEL_ID) return;

  const entries = Object.entries(USER_BIRTHDAYS).map(([id, { month, day }]) => `${id}:${month}-${day}`);
  const chunks = [];
  let current = [];
  let currentLen = 0;
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      const content = `BIRTHDAYS${i}:` + chunks[i].join(',');
      if (birthdayMessages[i]) {
        await birthdayMessages[i].edit(content);
      } else {
        birthdayMessages[i] = await channel.send(content);
      }
    }
    // List may have shrunk (e.g. after a rare edge case) - remove now-unused chunk messages
    for (let i = chunks.length; i < birthdayMessages.length; i++) {
      if (birthdayMessages[i]) await birthdayMessages[i].delete().catch(() => {});
    }
    birthdayMessages.length = chunks.length;
  } catch (err) {
    console.error('Failed to save birthdays to Discord:', err.message);
  }
}
...
// ---------------------------------------------------------------------------
function buildBirthdayMonthMessage() {
  const options = MONTH_NAMES_FULL.map((name, idx) => ({ label: name, value: String(idx + 1) }));
  const menu = new StringSelectMenuBuilder()
    .setCustomId('birthday_month')
    .setPlaceholder('Select your birth month')
    .addOptions(options);
  return {
    content: "🎂 We'd love to celebrate your birthday! Pick your birth month below (day comes next):",
    components: [new ActionRowBuilder().addComponents(menu)],
  };
}

function buildBirthdayDayComponents(month) {
  const daysInMonth = DAYS_IN_MONTH[month - 1];
  const makeOptions = (start, end) =>
    Array.from({ length: end - start + 1 }, (_, i) => ({ label: String(start + i), value: String(start + i) }));

  if (daysInMonth <= 25) {
    const menu = new StringSelectMenuBuilder()
      .setCustomId('birthday_day')
      .setPlaceholder(`Day (1-${daysInMonth})`)
      .addOptions(makeOptions(1, daysInMonth));
    return [new ActionRowBuilder().addComponents(menu)];
  }

  const firstCount = Math.ceil(daysInMonth / 2);
  const menuA = new StringSelectMenuBuilder()
    .setCustomId('birthday_day_a')
    .setPlaceholder(`Day (1-${firstCount})`)
    .addOptions(makeOptions(1, firstCount));
  const menuB = new StringSelectMenuBuilder()
    .setCustomId('birthday_day_b')
    .setPlaceholder(`Day (${firstCount + 1}-${daysInMonth})`)
    .addOptions(makeOptions(firstCount + 1, daysInMonth));
  return [new ActionRowBuilder().addComponents(menuA), new ActionRowBuilder().addComponents(menuB)];
}

async function sendBirthdayDM(member) {
  try {
    await member.send(buildBirthdayMonthMessage());
    return true;
  } catch (err) {
    console.error(`Failed to DM birthday picker to ${member.user.tag}:`, err.message);
    return false;
  }
}

// "!setup-birthdays" - leader-only, DMs every current member who hasn't
// already set a birthday. Paced with a short delay between sends to stay
// well clear of Discord's DM rate limits.
async function handleSetupBirthdaysCommand(message) {
  if (!isLeader(message)) {
    await message.reply('Only leaders can run this command.');
    return;
  }

  await message.reply('Starting birthday DM rollout - this may take a little while for the full member list...');
  ...
  for (const [id, member] of members) {
    ...
    if (USER_BIRTHDAYS[id]) { skipped++; continue; }
    const ok = await sendBirthdayDM(member);
    if (ok) sent++; else failed++;
    await new Promise(resolve => setTimeout(resolve, 1200)); // gentle pacing
  }

  await message.reply(`Done! Sent: ${sent}, already set (skipped): ${skipped}, failed (DMs likely closed): ${failed}.`);
}

// "!birthday" - anyone can run this anytime to set or redo their own birthday.
async function handleBirthdayCommand(message) {
  const ok = await sendBirthdayDM(message.member);
  if (ok) {
    await message.reply('Check your DMs! 🎂');
  } else {
    await message.reply("I couldn't DM you - please make sure DMs from server members are enabled, then try again.");
  }
}

// Daily check (server 09:00) - announces anyone whose real-calendar birthday
// is today, in BIRTHDAY_ANNOUNCE_CHANNEL_ID.
async function checkBirthdaysToday() {
  if (!BIRTHDAY_ANNOUNCE_CHANNEL_ID) return;

  const now = new Date();
  const month = now.getUTCMonth() + 1;
  const day = now.getUTCDate();

  const todaysBirthdays = Object.entries(USER_BIRTHDAYS).filter(([, bday]) => bday.month === month && bday.day === day);
  if (!todaysBirthdays.length) return;

  const channel = await client.channels.fetch(BIRTHDAY_ANNOUNCE_CHANNEL_ID);
  if (!channel) return;

  // Look up each person's display name for the ALL CAPS callout. If someone
  // left the server but their birthday entry hadn't been cleaned up yet
  // (shouldn't normally happen - guildMemberRemove handles that - but just
  // in case), fall back gracefully rather than erroring the whole message.
  const guild = channel.guild;
  const mentionParts = [];
  for (const [userId] of todaysBirthdays) {
    let displayName = 'MEMBER';
    try {
      const member = await guild.members.fetch(userId);
      displayName = (member.displayName || member.user.username).toUpperCase();
    } catch {
      // member not found - still mention the ID, just without a name
    }
    mentionParts.push(`<@${userId}> ${displayName}`);
  }

  let combinedNames;
  if (mentionParts.length === 1) {
    combinedNames = mentionParts[0];
  } else if (mentionParts.length === 2) {
    combinedNames = mentionParts.join(' AND ');
  } else {
    combinedNames = mentionParts.slice(0, -1).join(', ') + ', AND ' + mentionParts[mentionParts.length - 1];
  }

  const isPlural = mentionParts.length > 1;
  const content =
    `@everyone PLEASE JOIN ME IN WISHING ${combinedNames}, A VERY HAPPY BIRTHDAY, ` +
    `THANKS FOR BEING ${isPlural ? 'VALUED MEMBERS' : 'A VALUED MEMBER'} OF THE PACK. ` +
    `${WOLF_EMOJI} <:emoji_2:1525792577574539325>`;

  await channel.send({
    content,
    allowedMentions: { parse: ['everyone'], users: todaysBirthdays.map(([id]) => id) },
  });
}
...
// "!help birthday" - lists everything related to the birthday feature.
async function handleBirthdayHelpCommand(message) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `multiple same-day birthdays get combined into one message.`
    )
    .setColor(0xf687b3);
  await message.reply({ embeds: [embed] });
}
...
async function loadVipStateFromDiscord() {
  ...
  try {
    ...
    // VIPHISTORY:name1|date1,name2|date2,... - most recent first
    const historyMsg = ownMessages.find(m => m.content.startsWith('VIPHISTORY:'));
    if (historyMsg) {
      const raw = historyMsg.content.slice('VIPHISTORY:'.length);
      VIP_HISTORY = raw
        ? raw.split(',').map(entry => {
        ...
        : [];
      vipHistoryMessage = historyMsg;
      console.log(`Loaded ${VIP_HISTORY.length} VIP history entries from Discord storage.`);
    }

    // BIRTHDAYS0:, BIRTHDAYS1:, ... - chunked, gather all and sort by index
    // so they concatenate back together in the right order regardless of
    // fetch order.
    const birthdayMsgs = ownMessages
      .filter(m => /^BIRTHDAYS\d+:/.test(m.content))
      .sort((a, b) => {
        const numA = parseInt(a.content.match(/^BIRTHDAYS(\d+):/)[1], 10);
        const numB = parseInt(b.content.match(/^BIRTHDAYS(\d+):/)[1], 10);
        return numA - numB;
      });
    if (birthdayMsgs.length) {
      USER_BIRTHDAYS = {};
      birthdayMsgs.forEach(msg => {
        const raw = msg.content.replace(/^BIRTHDAYS\d+:/, '');
        if (!raw) return;
        raw.split(',').forEach(entry => {
          const [id, md] = entry.split(':');
          if (!id || !md) return;
          const [month, day] = md.split('-').map(Number);
          USER_BIRTHDAYS[id] = { month, day };
        });
      });
...
async function handleVipCommand(message) {
  ...
  const actualName = VIP_REMAINING[idx]; // properly-cased, in case the command used different casing
  VIP_REMAINING.splice(idx, 1);
  await saveVipRemainingToDiscord();

  VIP_HISTORY.unshift({ name: actualName, date: serverDateString(new Date()) });
  if (VIP_HISTORY.length > VIP_HISTORY_CAP) VIP_HISTORY.length = VIP_HISTORY_CAP;
  await saveVipHistoryToDiscord();
...
async function handleCaravanStatusCommand(message) {
  ...
  const historyLines = VIP_HISTORY.slice(0, 15).map(h => `${h.date} - ${h.name}`);
  const historyBody = historyLines.length ? historyLines.join('\n') : 'No VIPs picked yet.';

  await message.reply(
    `**🐫 Caravan Coachman schedule - next 7 days (upcoming only):**\n${body}\n\n` +
    `**Last ${historyLines.length} VIPs chosen (most recent first):**\n${historyBody}`
  );
}
...
// Cleans up birthday data when someone leaves the server - no point keeping
// it for someone no longer in the alliance. (The VIP list can't be cleaned
// up this way since it's stored by plain name, not Discord ID - there's no
// reliable link between a departing member and a VIP list entry.)
client.on('guildMemberRemove', async (member) => {
  if (USER_BIRTHDAYS[member.id]) {
    delete USER_BIRTHDAYS[member.id];
    await saveBirthdaysToDiscord().catch(err => console.error('Failed to save birthdays after member left:', err.message));
    console.log(`Removed birthday data for departed member: ${member.user.tag}`);
  }
});
  ...
  try {
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'birthday_month') {
        ...
        pendingBirthdayMonth.set(interaction.user.id, month);
        const dayComponents = buildBirthdayDayComponents(month);
        await interaction.update({
          content: `You picked **${MONTH_NAMES_FULL[month - 1]}** - now pick your day:`,
          components: dayComponents,
        });
        return;
      }

      if (['birthday_day', 'birthday_day_a', 'birthday_day_b'].includes(interaction.customId)) {
        const day = parseInt(interaction.values[0], 10);
        const month = pendingBirthdayMonth.get(interaction.user.id);
        if (!month) {
          await interaction.update({ content: 'Something went wrong - please run `!birthday` again to restart.', components: [] });
          return;
        }
        USER_BIRTHDAYS[interaction.user.id] = { month, day };
        pendingBirthdayMonth.delete(interaction.user.id);
        await saveBirthdaysToDiscord();
        await interaction.update({
          content: `✅ Got it! Your birthday is set to **${MONTH_NAMES_FULL[month - 1]} ${day}**. You can change this anytime with \`!birthday\`.`,
          components: [],
        });
        return;
      }
    ...
    if (approveMemberMatch || approveGuestMatch || denyMatch) {
      ...
      if (approveMemberMatch) {
        ...
        // Also invite them to set their birthday, same as existing members
        // can via "!birthday" - skip if they already have one somehow.
        if (!USER_BIRTHDAYS[targetMember.id]) {
          sendBirthdayDM(targetMember).catch(err => console.error('Failed to send birthday DM to new member:', err.message));
        }
...
client.on('messageCreate', async (message) => {
  ...
  try {
    ...
    } else if (content.startsWith('!setup-birthdays')) {
      await handleSetupBirthdaysCommand(message);
    } else if (content.startsWith('!birthdays')) {
...
client.once('ready', () => {
  ...
  // Birthday announcements: checked daily at server 09:00 - uses the real
  // calendar date (not server-time date) since birthdays are a real-world
  // concept, this just picks a consistent daily trigger time.
  const birthdayCheckUTCHour = serverHourToUTCHour(9);
  cron.schedule(`0 ${birthdayCheckUTCHour} * * *`, checkBirthdaysToday, { timezone: 'Etc/UTC' });
